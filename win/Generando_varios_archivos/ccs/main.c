...
char filename []= "BuffGPS.txt";
char nueva_registro [] ="\r\n Nuevo Registro \r\n";
//char cadena [] = "BuffGPS%.4i";

//char aaaaa[] = strcpy(filename, cadena);
//sprintf (filename, cadena , 1);
//filename = "BuffGPS";

//aux = printf();
//char prueba [] = "BuffGPS";
//aux = printf("%u",contador);
...
main(void)
{
	...
	for(n=0; n<sizeof(buferB); n++){
				UARTCharPut(UART1_BASE, buferB[n]);}
	...
	rc = f_open(&Fil, filename , FA_WRITE | FA_OPEN_ALWAYS);	//abre o crea un archivo
	rc = f_lseek(&Fil, Fil.fsize);
	rc = f_write(&Fil, nueva_registro, sizeof(nueva_registro), &bwGPS);
	rc = f_sync(&Fil);
	rc = f_close(&Fil);
