...
#if SSP_CH == 0
#define	SSPxDR		SSI0_DR_R
#define	SSPxSR		SSI0_SR_R
#define	SSPxCR0		SSI0_CR0_R
#define	SSPxCR1		SSP0CR1
#define	SSPxCPSR	SSP0CPSR
#define	CS_LOW()	//{FIO0CLR2 = _BV(2);}	/* Set P0.18 low */ //----CS PIN
#define	CS_HIGH()	//{FIO0SET2 = _BV(2);}	/* Set P0.18 high */ //----CS PIN
...
#define FCLK_FAST() {SSIConfigSetExpClk(SSI0_BASE, SysCtlClockGet(), SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 1000000, 8);} // { SSPxCPSR = (PCLK_SSP / SCLK_FAST) & ~1; }
#define FCLK_SLOW() {SSIConfigSetExpClk(SSI0_BASE, SysCtlClockGet(), SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 400000, 8);}//{ SSPxCPSR = (PCLK_SSP / SCLK_SLOW) & ~1; }
...
)
{
	...
	SSPxCR0 &= 0xFFF8;				/* Select 8-bit mode */
}
...
*/
	...
	*/
	//FIO0DIR |= _BV(18);	/* Set CS# as output */ //---CS PIN
#elif SSP_CH == 1
	/*
...
void disk_timerproc (void)
{
	...
	if (WP)		/* Write protected */
		...
		s &= ~STA_PROTECT;
	//if (INS)	/* Card is in socket */ //--- CD PIN
		s &= ~STA_NODISK;
	/*
	else		// Socket empty
