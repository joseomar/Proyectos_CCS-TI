...
static
BYTEC CardType;            /* b0:MMC, b1:SDC, b2:Block addressing */

static
BYTEC PowerFlag = 0;     /* indicates if "power" is on */
...
static
BOOL rcvr_datablock (
    BYTEC *buff,            /* Data buffer to store received data */
...
static
BOOL xmit_datablock (
    const BYTEC *buff,    /* 512 byte data block to be transmitted */
    BYTEC token            /* Data/Stop token */
)
{
    BYTEC resp, wc;
...
)
{
    ...
    xmit_spi((BYTEC)(arg >> 24));        /* Argument[31..24] */
    xmit_spi((BYTEC)(arg >> 16));        /* Argument[23..16] */
    xmit_spi((BYTEC)(arg >> 8));            /* Argument[15..8] */
    xmit_spi((BYTEC)arg);                /* Argument[7..0] */
    n = 0xff;
...
DSTATUS disk_initialize (
    BYTEC drv        /* Physical drive nmuber (0) */
)
{
    BYTEC n, ty, ocr[4];
	unsigned int i;
    ...
    if (send_cmd(CMD0, 0) == 1) {            /* Enter Idle state */
        Timer1 = 100000;                        /* Initialization timeout of 1000 msec */
...
DRESULT disk_read (
    BYTEC drv,            /* Physical drive nmuber (0) */
    BYTEC *buff,            /* Pointer to the data buffer to store read data */
    DWORD sector,        /* Start sector number (LBA) */
    BYTEC count            /* Sector count (1..255) */
)
{
...
DRESULT disk_write (
    BYTEC drv,            /* Physical drive nmuber (0) */
    const BYTEC *buff,    /* Pointer to the data to be written */
    DWORD sector,        /* Start sector number (LBA) */
    BYTEC count            /* Sector count (1..255) */
)
{
...
DRESULT disk_ioctl (
    BYTEC drv,        /* Physical drive nmuber (0) */
    BYTEC ctrl,        /* Control code */
    void *buff        /* Buffer to send/receive control data */
)
{
    DRESULT res;
    BYTEC n, csd[16], *ptr = buff;
    WORD csize;
